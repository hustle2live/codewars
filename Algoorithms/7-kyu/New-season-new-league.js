...
const premierLeagueStandings = (teamStandings) => {
   const firsTeam = teamStandings[1];

   const allTeamsFilteredAndSortArray = Object.values(teamStandings)
      .filter((team) => team !== firsTeam)
      .sort();

   const newTeamStandings = allTeamsFilteredAndSortArray.reduce(
      (acc, elem, ind, arr) => {
         acc[ind + 2] = elem;
         return acc;
      },
